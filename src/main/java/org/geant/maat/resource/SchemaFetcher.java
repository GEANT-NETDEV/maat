...
class SchemaFetcher {
    public Either<DomainError, JsonNode> fetch(String stringUrl) {
        ...
        try {
            ...
            return Either.left(DomainError.fromExceptionWhileAction(Error.SCHEMA_ERROR, e, "parsing url"));
        }
        try {
            return Either.right(mapper.readTree(url));
        } catch (IOException e) {
            return Either.left(DomainError.fromExceptionWhileAction(Error.SCHEMA_ERROR, e,
