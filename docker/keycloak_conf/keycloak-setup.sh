...
/opt/keycloak/bin/kcadm.sh create clients -r MaatRealm -s clientId=$KEYCLOAK_CLIENT_ID -s enabled=true -s clientAuthenticatorType=client-secret -s secret=$KEYCLOAK_CLIENT_SECRET -s standardFlowEnabled=true -s redirectUris=[\"http://172.18.187.252:8090\"]
/opt/keycloak/bin/kcadm.sh create clients -r MaatRealm -s clientId=maat-account -s enabled=true -s clientAuthenticatorType=client-secret -s secret=$KEYCLOAK_CLIENT_SECRET -s serviceAccountsEnabled=true -s standardFlowEnabled=false
